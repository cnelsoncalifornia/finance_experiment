...
class C(BaseConstants):
    NAME_IN_URL = 'practice_superstars'
    PLAYERS_PER_GROUP = None
    NUM_ROUNDS = 1
    CREDIT = 80 # This is sufficient to cover the maximum bidding price of 20 in each of the 4 bidding decisions per round.
    MAX = 20 # Maximum bid.
    NAMES = {
        1: ['I','D','B','D']  # In round 1, the highest asset is A in market 1 and the lowest asset is B.  The highest asset in market 2 is D and the lowest is A.
...
class Bid1(Page):
    def vars_for_template(player: Player):
       ...
       return dict(
          image_path1= 'practice_superstars/asset_movements_{}_part1.jpg'.format(player.round_number),
          image_path2='practice_superstars/asset_movements_{}_part1_graph.jpg'.format(player.round_number),
...
class Bid2(Page):
    def vars_for_template(player: Player):
        ...
        return dict(
            image_path1= 'practice_superstars/asset_movements_{}_part2.jpg'.format(player.round_number),
            image_path2='practice_superstars/asset_movements_{}_part2_graph.jpg'.format(player.round_number)
        )
    form_model = 'player'
...
class CombinedResults(Page):
    ...
    def vars_for_template(player: Player):
        ...
        return dict(
            image_path= 'practice_superstars/asset_movements_{}_part3.jpg'.format(player.round_number)
        )
